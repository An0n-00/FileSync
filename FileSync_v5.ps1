...
function FileSync
{
    ...
    if ( $syncconfirmation -eq "p" ) #if p
    {
        ...
        Write-Host "`n`nDo you want to sync, all contents (0) or the first level only (1)?" -ForegroundColor Yellow
        do
        {
        ...
        switch ( $syncoptions )
        {
            "00000"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $00000"
                Write-Host "`You are about to sync with the following Options:`n$00000"
                ...
                if ( $sync_check -eq "y" )
                {
                    ...
                    Invoke-Expression $robocopy_00000
            ...
            "00001"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $00001"
                Write-Host "`You are about to sync with the following Options:`n$00001"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_00001
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "00010"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $00010"
                Write-Host "`You are about to sync with the following Options:`n$00010"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_00010
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "00011"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $00011"
                Write-Host "`You are about to sync with the following Options:`n$00011"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_00011
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "00100"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $00100"
                Write-Host "`You are about to sync with the following Options:`n$00100"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_00100
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "00101"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $00101"
                Write-Host "`You are about to sync with the following Options:`n$00101"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_00101
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "00110"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $00110"
                Write-Host "`You are about to sync with the following Options:`n$00110"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_00110
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "00111"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $00111"
                Write-Host "`You are about to sync with the following Options:`n$00111"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_00111
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "01000"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $01000"
                Write-Host "`You are about to sync with the following Options:`n$01000"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_01000
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "01001"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $01001"
                Write-Host "`You are about to sync with the following Options:`n$01001"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_01001
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "01010"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $01010"
                Write-Host "`You are about to sync with the following Options:`n$01010"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_01010
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "01011"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $01011"
                Write-Host "`You are about to sync with the following Options:`n$01011"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_01011
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "01100"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $01100"
                Write-Host "`You are about to sync with the following Options:`n$01100"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_01100
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "01101"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $01101"
                Write-Host "`You are about to sync with the following Options:`n$01101"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_01101
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "01110"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $01110"
                Write-Host "`You are about to sync with the following Options:`n$01110"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_01110
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "01111"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $01111"
                Write-Host "`You are about to sync with the following Options:`n$01111"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_01111
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "10000"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $10000"
                Write-Host "`You are about to sync with the following Options:`n$10000"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_10000
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "10001"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $10001"
                Write-Host "`You are about to sync with the following Options:`n$10001"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_10001
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "10010"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $10010"
                Write-Host "`You are about to sync with the following Options:`n$10010"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_10010
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "10011"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $10011"
                Write-Host "`You are about to sync with the following Options:`n$10011"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_10011
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "10100"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $10100"
                Write-Host "`You are about to sync with the following Options:`n$10100"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_10100
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "10101"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $10101"
                Write-Host "`You are about to sync with the following Options:`n$10101"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_10101
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "10110"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $10110"
                Write-Host "`You are about to sync with the following Options:`n$10110"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_10110
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "10111"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $10111"
                Write-Host "`You are about to sync with the following Options:`n$10111"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_10111
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "11000"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $11000"
                Write-Host "`You are about to sync with the following Options:`n$11000"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_11000
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "11001"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $11001"
                Write-Host "`You are about to sync with the following Options:`n$11001"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_11001
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "11010"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $11010"
                Write-Host "`You are about to sync with the following Options:`n$11010"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_11010
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "11011"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $11011"
                Write-Host "`You are about to sync with the following Options:`n$11011"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_11011
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "11100"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $11100"
                Write-Host "`You are about to sync with the following Options:`n$11100"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_11100
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "11101"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $11101"
                Write-Host "`You are about to sync with the following Options:`n$11101"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_11101
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "11110"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $11110"
                Write-Host "`You are about to sync with the following Options:`n$11110"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_11110
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
            ...
            "11111"
            {
                log -logtype 1 -logMessage "Log: FileSync: Confirming to Sync with $11111"
                Write-Host "`You are about to sync with the following Options:`n$11111"
                $sync_check = Read-Host "`n`nAre you sure you want to proceed? (y)"
                if ( $sync_check -eq "y" )
                {
                    log -logtype 1 -logMessage "Log: FileSync: Syncing started"
                    Write-Host "Proceeding with the sync. This may take a while."
                    Write-Host "Warning: Do not close the script during the sync!" -ForegroundColor Red
                    Invoke-Expression $robocopy_11111
                    $syncpassed = $LASTEXITCODE
                    log -logtype 1 -logMessage "Log: FileSync: Syncing finished"
                }
                else
        ...
        switch ( $syncpassed )
        {
            ...
            8 {
                Write-Host "Several files didn't copy."
